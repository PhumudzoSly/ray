...
import { useFeatureLinks } from "@/hooks/use-feature-links";
...
export default function FeatureLinks({ featureId }: FeatureLinksProps) {
  const confirm = useConfirm();
  const [isAdding, setIsAdding] = useState(false);
  const { links, isLoading, createLink, isCreating, deleteLink, isDeleting } =
    useFeatureLinks(featureId);
  ...
  const handleCreateLink = async (values: z.infer<typeof formSchema>) => {
    ...
    try {
      const result = await createLink({ url: values.url, featureId });

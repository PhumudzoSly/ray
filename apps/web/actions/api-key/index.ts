...
export const getApiKey = async (id: string) => {
    ...
    try {
        const apiKey = await prisma.apiKey.findFirst({ where: { id, organizationId: org } });
        return { success: true, data: apiKey };
    } catch (error) {
        return { success: false, error };
    }
...
export const getAllApiKeys = async () => {
    const { org } = await getSession();
    try {
        const apiKeys = await prisma.apiKey.findMany({ where: { organizationId: org } });
 ...
 */
export const updateApiKey = async (id: string, data: Partial<{ name?: string; permissions?: any; isActive?: boolean; expiresAt?: Date }>) => {
    const { org } = await getSession();
    try {
        const apiKey = await prisma.apiKey.update({ where: { id, organizationId: org }, data });
        return { success: true, data: apiKey };
    } catch (error) {
        return { success: false, error };
    }

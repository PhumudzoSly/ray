...
function calculateProjectHealth(
    ...
    const issueCompletionRate = totalIssues > 0 ? (completedIssues / totalIssues) * 100 : 100;
    ...
    const deprecatedFeatures = features.filter(f => f.phase === 'DEPRECATED').length;

    const featureCompletionRate = totalFeatures > 0 ? (completedFeatures / totalFeatures) * 100 : 100;
    ...
    const milestoneCompletionRate = totalMilestones > 0 ? (completedMilestones / totalMilestones) * 100 : 100;
    ...
    if (atRiskMilestones > 0) {
        const riskPenalty = Math.min(25, (atRiskMilestones / totalMilestones) * 100);
    ...
    if (delayedMilestones > 0) {
        const delayPenalty = Math.min(20, (delayedMilestones / totalMilestones) * 80);
    ...
    if (inProgressMilestones > 0) {
        milestoneHealth += Math.min(10, (inProgressMilestones / totalMilestones) * 20);
    }
    ...
    if (healthScore >= 70) {
        if (issueCompletionRate > 80) healthFactors.unshift('High issue completion rate');
        if (featureCompletionRate > 60) healthFactors.unshift('Good feature progress');
        if (milestoneCompletionRate > 80) healthFactors.unshift('Milestones on track');
        if (inProgressFeatures > 0) healthFactors.unshift('Active feature development');
    }

    // Add warning factors for poor/critical health
    if (healthScore < 50) {
        if (blockedIssues > 0) healthFactors.unshift('Blocked issues need attention');
        if (atRiskMilestones > 0) healthFactors.unshift('Milestones at risk');
        if (inProgressFeatures === 0 && totalFeatures > 0) healthFactors.unshift('No features in development');
    }
...
export const getProjectHealthSummary = async (projectId: string) => {
    ...
    // Additional insights
    const criticalIssues = project.issues.filter(i => i.priority === 'CRITICAL').length;
    const highPriorityFeatures = project.features.filter(f => f.priority === 'CRITICAL' || f.priority === 'HIGH').length;
...
export const getProjectInsights = async (projectId: string) => {
    ...
    // Priority breakdowns
    const criticalIssues = project.issues.filter(i => i.priority === 'CRITICAL').length;
    const highPriorityIssues = project.issues.filter(i => i.priority === 'HIGH').length;
    const criticalFeatures = project.features.filter(f => f.priority === 'CRITICAL').length;
    const highPriorityFeatures = project.features.filter(f => f.priority === 'HIGH').length;

    // Due date analysis
    const now = new Date();
    const overdueIssues = project.issues.filter(i =>
        i.dueDate && new Date(i.dueDate) < now && i.status !== 'DONE'
    ).length;
    const upcomingDeadlines = project.issues.filter(i =>
        i.dueDate &&
        new Date(i.dueDate) > now &&
        new Date(i.dueDate) <= new Date(now.getTime() + 7 * 24 * 60 * 60 * 1000) &&
        i.status !== 'DONE'
    ).length;

    // Assignment analysis
    const unassignedIssues = project.issues.filter(i => !i.assignedToId).length;
    const unassignedFeatures = project.features.filter(f => !f.assignedToId).length;
    ...
    const recentFeatures = project.features.filter(f =>
        new Date(f.createdAt) >= sevenDaysAgo
    ).length;

    // Phase breakdown for features
    const discoveryFeatures = project.features.filter(f => f.phase === 'DISCOVERY').length;
    const planningFeatures = project.features.filter(f => f.phase === 'PLANNING').length;
    const developmentFeatures = project.features.filter(f => f.phase === 'DEVELOPMENT').length;
    const testingFeatures = project.features.filter(f => f.phase === 'TESTING').length;
    const liveFeatures = project.features.filter(f => f.phase === 'LIVE').length;

    // Status breakdown for issues
    const backlogIssues = project.issues.filter(i => i.status === 'BACKLOG').length;
    const inProgressIssues = project.issues.filter(i => i.status === 'IN_PROGRESS').length;
    const reviewIssues = project.issues.filter(i => i.status === 'REVIEW').length;
    ...
    return {
        ...
        issueStatuses: {
            ...
            review: reviewIssues,
            completed: healthMetrics.completedIssues,
            blocked: healthMetrics.blockedIssues,
            cancelled: healthMetrics.cancelledIssues
        }
    };
};
...
export const getOrganizationHealthInsights = async () => {
    ...
    const excellentProjects = projectHealths.filter(p => p.healthMetrics.healthStatus === 'excellent').length;
    const goodProjects = projectHealths.filter(p => p.healthMetrics.healthStatus === 'good').length;
    const fairProjects = projectHealths.filter(p => p.healthMetrics.healthStatus === 'fair').length;
    const poorProjects = projectHealths.filter(p => p.healthMetrics.healthStatus === 'poor').length;
    const criticalProjects = projectHealths.filter(p => p.healthMetrics.healthStatus === 'critical').length;

    const avgHealthScore = projectHealths.length > 0
        ? projectHealths.reduce((sum, p) => sum + p.healthMetrics.overallHealthScore, 0) / projectHealths.length
        : 0;
    ...
    const orgHealthMetrics = calculateProjectHealth(allIssues, allFeatures, allMilestones);
    ...
    return {
        ...
        needsAttention: projectHealths
            .filter(p => ['poor', 'critical'].includes(p.healthMetrics.healthStatus))
            .slice(0, 5)
    };
};

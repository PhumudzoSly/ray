 ...
 */
export const createFeatureRequest = async (data: { roadmapId: string; title: string; description: string; category: string; status: string; priority: string; isPublic: boolean; upvotes: number; name?: string; ipAddress?: string; adminNotes?: string }) => {
    ...
    try {
        ...
        const request = await prisma.featureRequest.create({ data: { ...data, userId } });
...
export const getFeatureRequest = async (id: string) => {
    ...
    try {
        const request = await prisma.featureRequest.findFirst({ where: { id, roadmap: { project: { organizationId: org } } } });
        return { success: true, data: request };
    } catch (error) {
        return { success: false, error };
    }
...
export const getAllFeatureRequests = async (roadmapId: string) => {
    ...
    try {
        ...
        if (!roadmap) return { success: false, error: 'Roadmap not found or not in your organization' };
        const requests = await prisma.featureRequest.findMany({ where: { roadmapId } });
 ...
 */
export const updateFeatureRequest = async (id: string, data: Partial<{ title?: string; description?: string; category?: string; status?: string; priority?: string; isPublic?: boolean; upvotes?: number; name?: string; ipAddress?: string; adminNotes?: string }>) => {
    const { org } = await getSession();
    try {
        // Ensure the request belongs to a roadmap in the org
        const request = await prisma.featureRequest.findFirst({ where: { id, roadmap: { project: { organizationId: org } } } });
        if (!request) return { success: false, error: 'Feature request not found or not in your organization' };

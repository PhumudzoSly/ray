 ...
 */
export const createRoadmapItem = async (data: { roadmapId: string; title: string; description: string; status: string; category: string; isPublic: boolean; priority: string; order: number; issueId?: string; nodeId?: string; targetDate?: Date }) => {
...
export const getRoadmapItem = async (id: string) => {
    const { org } = await getSession();
    try {
        const item = await prisma.roadmapItem.findFirst({ where: { id, roadmap: { project: { organizationId: org } } } });
        return { success: true, data: item };
    } catch (error) {
        return { success: false, error };
    }
 ...
 */
export const updateRoadmapItem = async (id: string, data: Partial<{ title?: string; description?: string; status?: string; category?: string; isPublic?: boolean; priority?: string; order?: number; issueId?: string; nodeId?: string; targetDate?: Date }>) => {
    const { org } = await getSession();
    try {
        // Ensure the item belongs to a roadmap in the org
        const item = await prisma.roadmapItem.findFirst({ where: { id, roadmap: { project: { organizationId: org } } } });
        if (!item) return { success: false, error: 'Item not found or not in your organization' };

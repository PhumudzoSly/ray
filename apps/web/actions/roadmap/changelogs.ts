...
export const getRoadmapChangelog = async (id: string) => {
    const { org } = await getSession();
    try {
        const changelog = await prisma.roadmapChangelog.findFirst({ where: { id, roadmap: { project: { organizationId: org } } } });
        return { success: true, data: changelog };
    } catch (error) {
        return { success: false, error };
    }
...
export const getAllRoadmapChangelogs = async (roadmapId: string) => {
    ...
    try {
        ...
        if (!roadmap) return { success: false, error: 'Roadmap not found or not in your organization' };
        const changelogs = await prisma.roadmapChangelog.findMany({ where: { roadmapId } });
 ...
 */
export const updateRoadmapChangelog = async (id: string, data: Partial<{ title?: string; description?: string; publishDate?: Date; isPublished?: boolean; items?: any }>) => {
    const { org } = await getSession();
    try {
        // Ensure the changelog belongs to a roadmap in the org
        const changelog = await prisma.roadmapChangelog.findFirst({ where: { id, roadmap: { project: { organizationId: org } } } });
        if (!changelog) return { success: false, error: 'Changelog not found or not in your organization' };
        const updated = await prisma.roadmapChangelog.update({ where: { id }, data });

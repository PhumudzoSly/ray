...
const searchTools = {
  ...
  search: createTool({
    ...
    handler: async ({ query, limit = 10, country, language }) => {
      try {
  ...
  searchDetailed: createTool({
    ...
    handler: async ({ query, maxResults = 5, includeContent = false }) => {
      try {
...
const researchTools = {
  ...
  research: createTool({
    ...
    handler: async ({ query, scrapeTopResults = 3, includeSummary = true }) => {
      try {
        const research = await webResearchService.research(
          {
            query,
  ...
  competitorResearch: createTool({
    ...
    handler: async ({ company, includeAnalysis = true }) => {
      try {
  ...
  trendResearch: createTool({
    ...
    handler: async ({ topic, timeframe = "month" }) => {
      try {
...
const analysisTools = {
  ...
  sentimentAnalysis: createTool({
    ...
    handler: async ({ topic, includeSources = false }) => {
      try {

  ...
  const analysisResult = await step?.ai.wrap(
    ...
    async () => {
      return await generateText({
        ...
        prompt: `
        You are a SaaS market research expert analyzing a specific search result in relation to a SaaS market research query.

        Here is the original SaaS idea we are researching:
        
        SAAS IDEA: ${topic}
        ${context ? `ADDITIONAL CONTEXT: ${context}` : ""}

        Current stage of SaaS market research:
        STAGE: ${stage.name}
        STAGE DESCRIPTION: ${stage.description}

        Here is the specific SaaS market research query we made to our search engine:
        QUERY: ${query}
        ...
        Provide a thoughtful SaaS market research analysis of this single search result that:
        1. Extracts the key SaaS market intelligence relevant to the query and overall SaaS idea
        2. Evaluates the credibility and relevance of the source for SaaS market research
        3. Identifies important SaaS market insights, data points, or perspectives provided
        4. Notes any limitations or biases in this particular source for SaaS validation
        5. Assesses how this information contributes to SaaS market validation

        Focus on SaaS-specific insights such as:
        - Market size and growth data
        - Target customer segment information
        - Competitive landscape details
        - Pricing and revenue model insights
        - Customer acquisition and retention strategies
        - Unit economics and business model data
        - Technology trends and platform opportunities
        - Acquisition potential and exit strategy information

        Make sure to include comprehensive SaaS market research details including any market data, customer insights, competitive intelligence, or business model information that we
        can use to validate the SaaS idea. Don't just summarize the result, but 
        provide a detailed SaaS market analysis of the result and how it contributes to our understanding of the SaaS market opportunity
        while also noting down important facts, market data, customer insights, and any other information that we can use
        to validate the SaaS business model.
        
        Your analysis should be focused specifically on this single result and what it contributes 
        to understanding the SaaS market validation query. Limit your analysis to about ${
            ...
            .array(
              z.object({
                query: z
                  .string()
                  .describe(
                    "The specific follow-up SaaS market research question to explore"
                  ),
                reasoning: z
                  .string()
                  .describe(
                    "Detailed reasoning behind why this follow-up SaaS market research query is important"
                  ),
              })
            )
            .length(maxBreadth)
            .describe(
              `Exactly ${maxBreadth} follow-up SaaS market research queries that will deepen the SaaS validation`
            ),
        }),
        prompt: `
        You are a SaaS market research expert generating follow-up queries based on initial SaaS market research findings.
        
        SAAS IDEA: ${topic}
        STAGE: ${stage.name}
        STAGE DESCRIPTION: ${stage.description}
        
        ORIGINAL SAAS MARKET RESEARCH QUERIES THAT HAVE ALREADY BEEN RESEARCHED:
        ${originalQueries.map((q, i) => `${i + 1}. ${q}`).join("\n")}
        
        SOURCES (use [n] inline when citing):
        ${referenceLines.join("\n")}
        
        Based on the following SaaS market research analyses from all findings (each prefixed with its citation number):
        ${analysesWithCites.join("\n\n")}
        
        Generate exactly ${maxBreadth} follow-up SaaS market research queries that will deepen the SaaS validation further.
        These queries should:
        1. Address important SaaS market gaps or open questions from the initial findings
        2. Explore promising SaaS market areas identified but not fully covered in the initial research
        3. Represent distinct SaaS validation angles that together provide comprehensive coverage
        4. Be specific enough for effective SaaS market intelligence web searches
        5. Include detailed reasoning explaining why each SaaS market research query is important
        6. NOT duplicate any of the original SaaS market research queries listed above
        7. Focus on SaaS-specific areas like market size, competitive landscape, pricing, customer acquisition, etc.
        
        The follow-up queries should not duplicate the initial SaaS market research, but rather build upon 
        and extend the SaaS market knowledge already gathered to provide deeper SaaS validation insights.
      `,
      });
    }
  );
  ...
  const analysisResult = await step?.ai.wrap(
    ...
    async () => {
      return await generateText({
        model: google("gemini-2.0-flash-001"),
        prompt: `
        You are a SaaS market research expert creating a comprehensive analysis for a SaaS market research stage.
        ...
        Generate a comprehensive SaaS market analysis of this entire research stage that:
        1. Synthesizes key SaaS market insights across all findings
        2. Draws parallels between each of the provided SaaS market findings/analyses
        3. Identifies SaaS market patterns, trends, and consensus views
        4. Highlights important SaaS market contradictions or areas of debate
        5. Evaluates the overall strength of SaaS market evidence
        6. Discusses implications of these findings for SaaS market validation
        7. Notes remaining SaaS market gaps or questions for future research
        8. Provides actionable SaaS market insights for business strategy

        Focus on SaaS-specific analysis areas such as:
        - Market size and growth potential assessment
        - Target customer segment validation
        - Competitive landscape analysis and differentiation opportunities
        - Pricing strategy and revenue model insights
        - Customer acquisition and retention strategy implications
        - Unit economics and business model viability
        - Technology trends and platform opportunities
        - Acquisition potential and exit strategy considerations

        Your analysis should provide a clear, coherent narrative of what we've learned about this aspect 
        of the SaaS market opportunity through our research. Structure this as a comprehensive multi-page SaaS market research report that 
        synthesizes all findings into a cohesive narrative suitable for SaaS decision-making. Your response should come close to but not exceed ${stageLevelLimit} 
        characters, as this represents the total volume of analysis being synthesized.

        When writing your SaaS market stage analysis, cite information inline using the IEEE style [n] where n corresponds 
 ...
 * ReasoningAgent
 *
 * This agent is responsible for building reasoning trees for each SaaS market research stage.
...
export const reasoningAgent = createAgent<NetworkState>({
  name: "SaaS Market Research Reasoning Agent",
  description: "Builds reasoning trees to explore SaaS market research stages",
  system: `You are an expert SaaS market research analyst building reasoning trees for exploring and validating SaaS ideas.
  
Your primary responsibility is to generate and expand SaaS market research queries that deeply explore the current reasoning stage.

When invoked, you will:
1. Check which stage of SaaS market research is currently active
2. Review the stage description to understand the SaaS validation focus
3. Use the 'build_reasoning_tree' tool to:
   - Research initial depth 0 queries with web searches for SaaS market intelligence
   - Generate follow-up queries based on initial SaaS market findings
   - Research these follow-up queries for deeper SaaS validation insights
   - Create a comprehensive SaaS market analysis for the stage

Your goal is to build a comprehensive SaaS market research foundation for each stage:
- First level: 3 initial SaaS market research queries with 3 search results each (9 total)
- Second level: 3 follow-up SaaS market queries with 3 search results each (9 more)
- Final analysis: Synthesize all 18 search results into a comprehensive SaaS market analysis

Focus on SaaS-specific research areas including:
...
Continue using the 'build_reasoning_tree' tool until the entire SaaS market research and analysis is complete.`,
  model: gemini({

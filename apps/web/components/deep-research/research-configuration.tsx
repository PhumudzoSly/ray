  ...
  return (
    <DropdownMenu open={isExpanded} onOpenChange={onToggleExpand}>
      <DropdownMenuTrigger asChild>
        <button className="flex items-center gap-2 py-2 px-3 rounded-full text-xs font-medium bg-zinc-100 dark:bg-zinc-800 text-zinc-700 dark:text-zinc-300 border border-zinc-300 dark:border-zinc-700 hover:bg-zinc-200 dark:hover:bg-zinc-700/50 transition-colors">
      ...
      <DropdownMenuContent align="start" className="w-64 p-4 space-y-4">
        {(Object.keys(CONFIG_LIMITS) as Array<keyof typeof CONFIG_LIMITS>).map(
          (key) => (

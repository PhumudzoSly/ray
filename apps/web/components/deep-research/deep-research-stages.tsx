...
export function DeepResearchStages({
  ...
  const getStageProgress = (stage: ResearchStage) => {
    ...
    const nodesWithFindings = stage.reasoningTree.nodes.filter(
      (n) => n.findings.length > 0
    ).length;

    return totalNodes > 0
      ? Math.round((nodesWithFindings / totalNodes) * 100)
      : 0;
  };
                ...
                <div className="ml-4 flex-1">
                  ...
                  {(currentNodes > 0 || expectedNodes > 0) && (
                    <div className="mt-2 flex items-center gap-4 text-xs text-zinc-500 dark:text-zinc-400">
                      <span>
                        Nodes: {currentNodes}/{expectedNodes}
                      </span>
                      {stage.reasoningTree?.nodes && (
                        <span>
                          With Findings:{" "}
                          {
                            stage.reasoningTree.nodes.filter(
                              (n) => n.findings.length > 0
                            ).length
                          }
                        </span>

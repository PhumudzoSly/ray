...
import { DeepResearchMessage } from "./deep-research-message";
...
import { ResearchConfiguration } from "./research-configuration";
...
export function DeepResearchChat() {
  ...
  const isDeepResearchEnabled = () => {
    const currentWorkflow = workflowOptions.find(
  ...
  const updateCustomScrollbar = () => {
    ...
    const thumbHeight = Math.max(
      20,
      (textarea.clientHeight / textarea.scrollHeight) * textarea.clientHeight
    );
    ...
    if (textarea.scrollHeight > textarea.clientHeight) {
      ...
      const thumbPosition =
        scrollPercentage * (textarea.clientHeight - thumbHeight);
  ...
  const handleTrackClick = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    const percentage =
      (relativeY - thumbHeight / 2) / (textarea.clientHeight - thumbHeight);
                  ...
                  setStages((prev) => {
                    ...
                    const updatedStages = updateData.stages!.map((newStage) => {
                      ...
                      return {
                        ...
                        reasoningTree:
                          newStage.reasoningTree || existing?.reasoningTree,
                        analysis: newStage.analysis || existing?.analysis,
                        reasoningComplete:
                          newStage.reasoningComplete ??
                          existing?.reasoningComplete ??
                          false,
                        analysisComplete:
                          newStage.analysisComplete ??
                          existing?.analysisComplete ??
                          false,
                      };
                    });
                                      ...
                                      if (existingNode) {
                                        ...
                                        if (
                                          node.findings.length >
                                            existingNode.findings.length ||
                                          node.children.length >
                                            existingNode.children.length ||
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        >
          ...
          {updates.length === 0 ? (
            ...
            <>
              {/* User query */}
              <DeepResearchMessage role="user" content={userQuery} />
              ...
              {/* Final report */}
              {finalReport && (
                <DeepResearchMessage role="assistant" content={finalReport} />
              )}
            </>
          )}
                  ...
                  {isDeepResearchEnabled() && (
                    <div className="flex items-center py-2 px-3 rounded-full text-xs font-medium bg-violet-100 dark:bg-violet-900/20 text-violet-700 dark:text-violet-400 border border-violet-300 dark:border-violet-800/30">
                      ...
                      <Brain
                        ...
                        style={{
                          filter:
                            "drop-shadow(0 0 2px rgba(219, 39, 119, 0.5))",

...
import { DeepResearchStages } from "./deep-research-stages";
import {
  Tooltip,
  TooltipContent,
  TooltipProvider,
                        ...
                        <div className="space-y-2 p-2">
                          <div>
                            ...
                            <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-xs text-white/80">
                              ...
                              <span>
                                Queries/Stage: {configuration.queriesPerStage}
                              </span>
                              <span>Max Depth: {configuration.maxDepth}</span>
                              <span>
                                Max Breadth: {configuration.maxBreadth}
                              </span>
                            </div>
                          </div>
                          {latestUpdate?.tree && expectedTotalNodes > 0 && (
                            <div>
                              ...
                              <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-xs text-white/80">
                                <span>
                                  Nodes: {latestUpdate.tree.nodeCount || 0}/
                                  {expectedTotalNodes}
                                </span>
                                <span>
                                  With Findings:{" "}
                                  {latestUpdate.tree.nodesWithFindings || 0}
                                </span>
                                <span>
                                  Current Depth:{" "}
                                  {latestUpdate.tree.maxDepth || 0}/
                                  {configuration.maxDepth}
                                </span>
                                <span>
                                  Completion:{" "}
                                  {(
                                    ((latestUpdate.tree.nodesWithFindings ||
                                      0) /
                                      expectedTotalNodes) *
                                    100
                                  ).toFixed(0)}

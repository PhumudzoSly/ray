...
export function RiskAnalysis({ overallRiskScore, riskItems = [] }: RiskAnalysisProps) {
  ...
  const categoryCounts = (riskItems || []).reduce((acc, item) => {
        ...
        <CardContent>
          <div className="text-4xl font-bold text-center py-4">
            {overallRiskScore !== null && overallRiskScore !== undefined ? 
              `${overallRiskScore.toFixed(0)}/100` : "N/A"}
          </div>
          <div className="text-center text-sm text-muted-foreground">
            {overallRiskScore !== null && overallRiskScore !== undefined ? (
              overallRiskScore < 30 ? "Low Risk" : 
              overallRiskScore < 60 ? "Moderate Risk" : 
      ...
      {(riskItems || []).length > 0 ? (
        ...
        <Card className="lg:col-span-2">
          ...
          <CardContent className="h-64 flex items-center justify-center">
            <p className="text-muted-foreground">No risk matrix data available</p>
          </CardContent>
        </Card>
      )}
